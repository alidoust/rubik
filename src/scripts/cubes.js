	...
	document.addEventListener( 'keypress', function( event ){
		if( event.target.tagName.toLowerCase() !== 'input' &&
			...
			this.keyboardControlsEnabled ){
				...
				if( 'XxRrMmLlYyUuEeDdZzFfSsBb'.indexOf( old_key ) >= 0 ) this.twist( 'XxRrMmLlYyUuEeDdZzFfSsBb'.indexOf( old_key ) );
...
ERNO.extend( ERNO.Cube.prototype, {
	...
	immediateTwist: function( twist ){
		...
		if (twist.is_view_space === true) {
			mappings = {};
			mappings['x'] = this.slicesDictionary['x'];
			mappings['y'] = this.slicesDictionary['y'];
			mappings['z'] = this.slicesDictionary['z'];

			for (var i=0; i < 9; i++) {
				var slice = this.slices[i];
				var normal = slice.origin.position.clone();
				...
				if (normal.equals(ERNO.Direction.UP.normal)) {
					mappings['u'] = slice;
					mappings['e'] = slice.neighbour;
				} else if (normal.equals(ERNO.Direction.DOWN.normal)) {
					mappings['d'] = slice;
				} else if (normal.equals(ERNO.Direction.LEFT.normal)) {
					mappings['l'] = slice;
					mappings['m'] = slice.neighbour;
				} else if (normal.equals(ERNO.Direction.RIGHT.normal)) {
					mappings['r'] = slice;
				} else if (normal.equals(ERNO.Direction.FRONT.normal)) {
					mappings['f'] = slice;
					mappings['s'] = slice.neighbour;
				} else if (normal.equals(ERNO.Direction.BACK.normal)) {
					mappings['b'] = slice;
				} else {
					console.log('wtf?');
				}
