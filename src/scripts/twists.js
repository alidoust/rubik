...
ERNO.Twist = function( command, degrees, is_view_space ){

	if( command ) this.set( command, degrees, is_view_space )

}


ERNO.Twist.prototype.set = function( command, degrees, is_view_space ){
	...
	if( group !== undefined ){
		...
		this.is_view_space = is_view_space || false;
...
ERNO.Twist.prototype.copy = function( twist ){
	...
	this.is_view_space = twist.is_view_space;
	...
	for( i = 0; i < elements.length; i ++ ){
		...
		if( element instanceof ERNO.Twist ){
			...
			if( typeof lookAhead === 'number' ){
					elements[ i ] = new ERNO.Twist( 'XxRrMmLlYyUuEeDdZzFfSsBb'[element], lookAhead, true );
			}
			else elements[ i ] = new ERNO.Twist( 'XxRrMmLlYyUuEeDdZzFfSsBb'[element], undefined, true );
		}
